...
deploy:
	...
	ssh ${HOST} -p ${PORT} 'cd todo_${BUILD_NUMBER} && docker-compose run api-php-cli openssl genrsa -out private.key 2048'
	ssh ${HOST} -p ${PORT} 'cd todo_${BUILD_NUMBER} && docker-compose run api-php-cli openssl rsa -in private.key -pubout -out public.key'
