...
deploy:
	...
	ssh ${HOST} -p ${PORT} 'cd todo_${BUILD_NUMBER} && echo "VUE_APP_API_URL=${VUE_APP_API_URL}" >> .env'
	...
	ssh ${HOST} -p ${PORT} 'cd todo_${BUILD_NUMBER} && docker-compose run api-php-cli openssl genrsa -out /var/www/api/private.key 2048'
	ssh ${HOST} -p ${PORT} 'cd todo_${BUILD_NUMBER} && docker-compose run api-php-cli openssl rsa -in /var/www/api/private.key -pubout -out /var/www/api/public.key'
